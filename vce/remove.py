...
# Remove Tox21 assay columns (NR-*, SR-*)
tox21_assay_cols = [col for col in merged.columns
                    if col.startswith('NR-') or col.startswith('SR-')]

# Remove duplicate smiles column from Tox21
extra_cols = ['smiles', 'mol_id']

# Remove all unwanted columns
cols_to_drop = tox21_assay_cols + [
    col for col in extra_cols if col in merged.columns
]
merged.drop(columns=cols_to_drop, inplace=True)

print(f"\nRemoved {len(cols_to_drop)} unwanted columns")
